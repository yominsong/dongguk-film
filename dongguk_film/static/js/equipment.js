...
function executeWhenPurposeIsSelected(selectedPurpose = null) {
    ...
    if (selectedPurpose) {
        ...
        setTimeout(() => { yAxisScrollPositionValueForModal = id_scrollable_part_of_modal.scrollTop }, 500); // When the behavior option of the scrollTo() method is set to smooth, the duration varies across web browsers, so a random estimate of 300ms is applied to setTimeout()
    } else {
